...
## Validation 및 Exception Handling (Issue #5)
...
실패 시나리오를 `supertest`로 자동 검증합니다. Postman 수동 테스트와 병행할 수 있습니다.
...
### Postman 수동 테스트 (Issue #5 체크리스트)

| # | 시나리오 | Method | URL | 기대 |
|---|----------|--------|-----|------|
| 1 | signup validation | POST | `/auth/signup` | 400 |
| 2 | 이메일 중복 | POST | `/auth/signup` | 409 |
| 3 | login 실패 | POST | `/auth/login` | 401 |
| 4 | JWT 없이 글쓰기 | POST | `/posts` | 401 |
| 5 | title/content 누락 | POST | `/posts` | 400 |
| 6 | page=abc | GET | `/posts?page=abc` | 400 |
| 7 | 없는 URL | GET | `/unknown-route` | 404 |

Body는 **raw → JSON**, `POST /posts`는 **Authorization → Bearer Token**에 login에서 받은 `accessToken`을 설정합니다.
...
| `dto/signup.dto.ts` | 회원가입 요청 DTO (`@IsNotEmpty`, `@IsEmail`, `@IsString`, `@MinLength(8)`) |
| `dto/login.dto.ts` | 로그인 요청 DTO (`@IsNotEmpty`, `@IsEmail`, `@IsString`, `@MinLength(8)`) |
...
| 파일 | 역할 |
|------|------|
| `app.e2e-spec.ts` | `GET /` 기본 E2E 테스트 |
| `auth.e2e-spec.ts` | Auth signup/login 실패 시나리오 E2E |
| `jwt-auth.e2e-spec.ts` | JWT 인증 실패 시나리오 E2E (`POST /posts`) |
| `posts-create.e2e-spec.ts` | 게시글 작성 body validation E2E |
| `posts-query.e2e-spec.ts` | 게시글 목록 query validation E2E |
| `route-not-found.e2e-spec.ts` | 존재하지 않는 Route 404 E2E |
| `jest-e2e.json` | E2E Jest 설정 (`maxWorkers: 1`로 SQLite 충돌 방지) |
...
- Flutter 프론트엔드 (로그인/회원가입, 게시글 목록, 글쓰기) (높은 우선순위)
